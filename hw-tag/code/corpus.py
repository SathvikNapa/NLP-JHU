...
class Sentence(List[TWord]):
    ...
    def __str__(self) -> str:
        return " ".join(
            [word if tag is None else f"{word}/{tag}" for (word, tag) in self[1:-1]]
        )
    ...
    def desupervise(self):
        ...
        sentence = Sentence()
        [
            sentence.append((word, tag if tag == BOS_TAG or tag == EOS_TAG else None))
            for word, tag in self
        ]
        return sentence
        ...
        *files: Path,
        ...
        if tagset is None or vocab is None:
            ...
            for word, tag in self.get_tokens(oovs=False):
                ...
                word_counts[
                    word
                ] += 1  # count words to see later if we pass the threshold
                if tag is not None:
                    self.tagset.add(tag)  # no threshold for tags
            log.info(
                f"Read {sum(word_counts.values())} tokens from {', '.join(file.name for file in files)}"
            )
            # log.info(
        ...
        if tagset is not None or vocab is not None:
            for word, tag in self.get_tokens(oovs=False):
                self.integerize_word(
                    word
                )  # make sure it doesn't throw exception with given vocab
                if tag is not None:
                    self.integerize_tag(
                        tag
                    )  # make sure it doesn't throw exception with given tagset
                    ...
                    for token in line.split():
                        ...
                        if "/" in token:
                            ...
                            word, tag = (
                                Word(token),
                                None,
                            )  # for example, "caviar" without a tag
    ...
    def integerize_tag(self, tag: Tag) -> int:
        ...
        if t is None:
            raise KeyError(
                tag,
                self,
                f"This tag isn't in the tagset of this corpus (we don't support OOV tags)",
            )
        return t
        ...
        if w is None:
            ...
            if w is None:  # OOV_WORD needs to be in our tagset
                raise KeyError(
                    word,
                    self,
                    f"This word isn't in the vocab of the corpus, and can't be converted to OOV since OOV is also missing from the vocab",
                )
        return w

    def integerize_tword(self, tword: TWord) -> IntegerizedTWord:
        word, tag = tword
        return self.integerize_word(word), (
            None if tag is None else self.integerize_tag(tag)
        )

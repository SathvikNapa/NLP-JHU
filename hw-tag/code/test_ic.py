...
log.info(
    "*** A, B matrices after training on icsup (should "
...
log.info(
    "*** Forward algorithm on icraw (should approximately match iteration 0 "
    "on spreadsheet)"
)
...
cross_entropy_loss = lambda model: model_cross_entropy(model, icsup)
crf.train(corpus=icsup, loss=cross_entropy_loss, lr=0.1, reg=0.01, tolerance=0.0001)
...
icraw = TaggedCorpus(Path("icraw"), tagset=icsup.tagset, vocab=icsup.vocab)
write_tagging(
    crf, icraw, Path("icraw_crf.output")
)  # calls hmm.viterbi_tagging on each sentence

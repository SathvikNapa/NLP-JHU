...
class ConditionalRandomField(HiddenMarkovModel):
    ...
    def init_params(self) -> None:
        ...
        self.WA = 0.01 * torch.rand(n_rows, self.k)
        self.WB = 0.01 * torch.rand(self.k, self.V)

        self.WA[:, self.bos_t] = NEGINF
        if not self.unigram:
            self.WA[self.eos_t, :] = NEGINF

        self.WB[self.bos_t, :] = NEGINF
        self.WB[self.eos_t, :] = NEGINF
        ###
    ...
    def train(
        ...
        *,
        ...
        reg: float = 0.0,
        max_steps: int = 50000,
        save_path: Optional[Path | str] = "my_crf.pkl",
    ) -> None:
        ...
        if reg < 0:
            raise ValueError(f"{reg=} but should be >= 0")
        if minibatch_size <= 0:
            raise ValueError(f"{minibatch_size=} but should be > 0")
        if minibatch_size > len(corpus):
            minibatch_size = len(
                corpus
            )  # no point in having a minibatch larger than the corpus
        ...
        for evalbatch in more_itertools.batched(
            itertools.islice(
                corpus.draw_sentences_forever(), max_steps
            ),  # limit infinite iterator
            eval_interval,
        ):  # group into "evaluation batches"
            for sentence in tqdm(evalbatch, total=eval_interval):
                ...
                if steps % minibatch_size == 0:
                    ...
                    learning_speeds.append(
                        self.learning_speed(lr, minibatch_size)
                    )  # add this minibatch's learning speed to list
            ...
            if not isnan(sum(learning_speeds)):
                logger.info(
                    f"Average learning speed: {sum(learning_speeds)/len(learning_speeds):.2g} (estimated training loss reduction per example)"
                )
            learning_speeds = []
    ...
    def logprob(self, sentence: Sentence, corpus: TaggedCorpus) -> TorchScalar:
        ...
        log_score_joint = self.forward_pass(isent)
        log_z = self.forward_pass(desup_isent)

        return log_score_joint - log_z

    def accumulate_logprob_gradient(
    ...
    def reg_gradient_step(self, lr: float, reg: float, frac: float):
        ...
        if reg == 0:
            return  # can skip this step if we're not regularizing
        ...
        decay_f = 1.0 - (lr * reg * frac)
        if decay_f < 0:
            decay_f = 0

        with torch.no_grad():
            self.WA.mul_(torch.where(torch.isinf(self.WA), 1.0, decay_f))
            self.WB.mul_(torch.where(torch.isinf(self.WB), 1.0, decay_f))

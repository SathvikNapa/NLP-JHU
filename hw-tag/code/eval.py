...
def viterbi_tagger(
    model: HiddenMarkovModel, eval_corpus: TaggedCorpus
) -> Callable[[Sentence], Sentence]:
...
def model_cross_entropy(model: HiddenMarkovModel, eval_corpus: TaggedCorpus) -> float:
    ...
    log.info(
        f"Cross-entropy: {cross_entropy:.4f} nats (= perplexity {exp(cross_entropy):.3f})"
    )
    return cross_entropy


def viterbi_error_rate(
    ...
    if show_cross_entropy:
        model_cross_entropy(
            model, eval_corpus
        )  # call this for its side effect (logging)
    return tagger_error_rate(
        viterbi_tagger(model, eval_corpus), eval_corpus, known_vocab=known_vocab
    )


def tagger_error_rate(
    tagger: Callable[[Sentence], Sentence],
    eval_corpus: TaggedCorpus,
    known_vocab: Optional[Integerizer[Word]] = None,
) -> float:
...
def eval_tagging(
    predicted: Sentence, gold: Sentence, known_vocab: Optional[Integerizer[Word]]
    ...
    for (word, tag), (goldword, goldtag) in zip(predicted, gold):
        assert (
            word == goldword or word == OOV_WORD
        )  # sentences being compared should have the same words!
...
def write_tagging(
    model_or_tagger: Union[HiddenMarkovModel, Callable[[Sentence], Sentence]],
    eval_corpus: TaggedCorpus,
    output_path: Path,
    tagger_method: str = "viterbi",
) -> None:
    if isinstance(model_or_tagger, HiddenMarkovModel):
        if tagger_method in ("viterbi", "viterbi_decoding"):

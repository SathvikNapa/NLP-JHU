...
def parse_args() -> argparse.Namespace:
    ...
    filegroup.add_argument(
        "input",
        type=str,
        help="input sentences for evaluation (labeled dev set or test set)",
    )
    ...
    if args.model:
        if args.train:
            ...
            if os.path.exists(args.model):
                log.warning(
                    f"Warning: {args.model} already exists; will be overwritten at end of training"
                )
                if not args.checkpoint:
                    log.warning(
                        f"Warning: won't load {args.model}: use --checkpoint for that"
                    )
        else:
        ...
        log.warning(
            f"Warning: model won't be saved at end of training (use --model for that)"
        )
    ...
    if not args.eval_file:
        log.warning(
            f"Warning: won't save evaluation messages (use --eval_file for that)"
        )
    ...
    if args.load_path:  # don't need to create a new model
        ...
        if args.rnn_dim or args.lexicon or args.problex:
            raise NotImplementedError(
                "No neural HMM implemented (possible, but not required for these homeworks)"
            )
        else:
            args.new_model_class = HiddenMarkovModel
    else:  # create some sort of CRF
        if args.rnn_dim or args.lexicon or args.problex:
            from crf_neural import (
                ConditionalRandomFieldNeural,
            )  # module provided with hw-rnn homework
...
def main() -> None:
    ...
    logging.basicConfig(
        level=logging.DEBUG
    )  # log everything, and install default handler (print to stderr)
    ...
    if args.load_path:
        ...
        model = HiddenMarkovModel.load(
            args.load_path, device=args.device
        )  # HMM is ancestor of all classes
        ...
        train_corpus = TaggedCorpus(
            *train_paths, tagset=model.tagset, vocab=model.vocab
        )
    else:
        ...
        if not getattr(new_model_class, "neural", False):
            ...
            model = new_model_class(
                train_corpus.tagset, train_corpus.vocab, unigram=args.unigram
            )
        else:
            ...
            if args.lexicon:
                ...
                known_vocab = (
                    train_corpus.vocab
                )  # save training vocab, since it may be replaced
                lexicon = build_lexicon(
                    train_corpus,
                    embeddings_file=Path(args.lexicon) if args.lexicon else None,
                    newvocab=TaggedCorpus(
                        Path(args.input)
                    ).vocab,  # add only eval words from file
            ...
            model = new_model_class(
                ...
                lexicon=lexicon,  # neural model args
    ...
    other_loss = lambda x: viterbi_error_rate(
        x,
        eval_corpus,
    ...
    if train_corpus:
        if isinstance(model, ConditionalRandomField):
            for option in "λ":
                if getattr(args, option):
                    log.warning(
                        f"Ignoring --{option} since we're training by SGD and don't have an M step "
                    )
            model.train(
                corpus=train_corpus,
                loss=loss,
    ...
    with torch.inference_mode():  # turn off all gradient tracking
        ...
        loss(
            model
        )  # show the main loss function (using the logger) -- redundant if we trained

...
from probs_v2 import read_vocab, UniformLanguageModel, AddLambdaLanguageModel, \
...
def get_model_filename(args: argparse.Namespace) -> Path:
    prefix = f"new_models/corpus={args.train_file.name}~vocab={args.vocab_file.name}~smoother={args.smoother}"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Initial learning rate for log-linear models (default 0.01)",
    )
...
def main():
    ...
    if args.smoother == UNIFORM:
        ...
        if args.lexicon is None:
            raise Exception(f"{args.smoother} requires a lexicon")
        lm = EmbeddingLogLinearLanguageModel(vocab, args.lexicon, args.l2_regularization, args.epochs, args.lr)
